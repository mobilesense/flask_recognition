...
void gradAngle(DARY *im, DARY *dx, DARY *dy, DARY *grad, DARY *ori){
  ...
  for (uint i = 0; i < im->size(); i++){
    fdx=dx->fel[0][i];
    fdy=dy->fel[0][i];
    grad->fel[0][i] = sqrt( fdx*fdx  +  fdy*fdy );
    ori->fel[0][i] = atan2 (fdy, fdx);
  }
} 
...
void gradAngle(DARY *im, DARY *grad, DARY *ori){
  ...
  for (uint i = 0; i < im->size(); i++){
    dx=grad->fel[0][i];
    dy=ori->fel[0][i];
    grad->fel[0][i] = sqrt( dx*dx  +  dy*dy );
    ori->fel[0][i] = atan2 (dy, dx);
  }
