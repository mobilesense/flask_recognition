...
ImageContent::ImageContent(const char * name, int in) {
#ifndef NOIMAGEMAGICK    
  Magick::Image img;
  img.read(name);
  
  initFloat( img.rows() , img.columns() );
     for (uint i=0;i<img.rows();i++){
       for (uint j=0;j<img.columns();j++){
           fel[i][j] = (int) (Magick::ColorGray(img.pixelColor(j,i)).shade() * 256);
       }
   }
#else 

  fprintf(stderr, "cannot load %s: ImageMagick not compiled in\n", name); 
  exit(10);
...
void ImageContent::scaleHalf(const DARY *im_in){
    float px, py, dx, dy;
    int arondx, arondy;
    int xim_in=im_in->x()-1;
    int yim_in=im_in->y()-1;

    assert(2*(x_size-1)<im_in->x());
    assert(2*(y_size-1)<im_in->y());
    
    for(uint j=0;j<y_size; j++){
	for(uint i=0;i<x_size; i++){
          fel[j][i]=0.25*(im_in->fel[2*j  ][2*i  ]+
                          im_in->fel[2*j  ][2*i+1]+
                          im_in->fel[2*j+1][2*i  ]+
                          im_in->fel[2*j+1][2*i+1]);
	}
    } 
}
